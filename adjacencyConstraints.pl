...
checkAdjacency(SpaceVarList):-
	findall(X, adj(X, _), AdjList),
	checkAdjacency(AdjListName, SpaceVarList).

checkAdjacency([], _).

checkAdjacency([Name1 | Names], SpaceVarList):-
	getSpaceVarFromName(Name1, SpacevarList, SpaceVar1),
	adj(Name1, Name2),
	getSpaceVarFromName(Name2, SpaceVarList, SpaceVar2),
	getCoordinates(SpaceVar1, Coord1),
	getCoordinates(SpaceVar2, Coord2),
